...
from model import MLP, GCN, SAGE, LinkPredictor
...
from teacher_main import test_transductive
import os
...
def test(model, predictor, data, split_edge, evaluator, batch_size, encoder_name, dataset, args):
    model.eval()
    predictor.eval()
...
def main():
    ...
    pretrained_model = torch.load(
        "./saved-models/" + args.datasets + "-" + "sage" + "_" + args.transductive + ".pkl")
    ...
    t_h = torch.load("./saved-features/" + args.datasets + "-" + "sage" + "_" + args.transductive + ".pkl")
    t_h = t_h['features']
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            if args.transductive == "transductive":
                ### python main.py --minibatch表示使用minibatch
                if args.minibatch:
                    loss = train_minibatch(model, predictor, t_h, teacher_predictor, data, split_edge, optimizer, args,
                                           device)

                else:
                    loss = train(model, predictor, t_h, teacher_predictor, data, split_edge, optimizer, args, device)

                    results, h = test(model, predictor, data, split_edge, evaluator, args.link_batch_size,
                                               args.encoder, args.datasets,args)
